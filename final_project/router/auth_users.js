...
const isValid = (username) => {
  // true = username is available (not already registered)
...
// Task 8: Add or modify a book review (auth required)
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  if (!review) {
    return res.status(404).json({ message: "Review text is required as a query parameter (?review=...)." });
  }
...
// Task 9: Delete a book review (auth required, only own review)
